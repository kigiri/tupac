...
window.__tupac_get_default__ = _ => _ && (_.__esModule ? _.default : _)
...
const style = errorDisplay.style

const errorList = []
const displayError = (err, mod) => {
  console.error(err)
  errorList.push(errorDisplay.textContent = `
${mod && (mod.fullPath || mod.key)}

${err.stack}`)
  let showError = errorList.length - 1
  errorDisplay.onclick = () => {
    showError--
    if (showError < 0) {
      showError = errorList.length
    }
    errorDisplay.textContent = errorList[showError]
  }
...
const loadScript = (mod, script) => {
  ...
  return (mod.load = () => Promise.all(requires.map(exec))
    .then(args => {
      (typeof load === 'string') && (load = new Function(names, load))
...
const require = key => {
  ...
  return modules[key].ready = retryOnError()
    ...
    .catch(err => displayError(err, mod))

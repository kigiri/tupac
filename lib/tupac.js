...
const logger = {
  ...
  request: ({ method, url }, res, error) => {
    if (!error) return logger.info('%s %s', method.yellow, url.cyan)
    const { status, message } = error
    logger.info(`${method.red} ${url.red} Error (${String(status).red}): ${message.red}`)
  }
}
...
const startTupac = (port, extraOptions) => {
  ...
  logger.info('watching for changes on ' + root.vert)
